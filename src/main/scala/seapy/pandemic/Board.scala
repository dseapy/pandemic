...
class Board {
  val cities = Board.initalCities()
  var infectionRateStage = 0
  var outbreaks = 0

  def infectionRate(): Int = {
    if(infectionRateStage < 3) 2
    else if(infectionRateStage < 5) 3
    else 4
  }

  def executeEpidemic(){
    //1-Increase: Move the infection Rate marker forward 1 space.
    infectionRateStage = infectionRateStage + 1
    //2-Infect: Draw the bottom card from the infection deck and put 3 cubes on that city.  Discard that card.
    //3-Intensify: Shuffle the cards in the infection discard pile and put them on top of the infection deck.
  }
}
